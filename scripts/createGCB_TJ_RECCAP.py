### script for producing global carbon budget output, version 2023
### this version updated by Tereza Jarníková, based heavily on Dave Willis's /gpfs/data/greenocean/GCB/GCB_RECCAPcreateGCB_RECCAP.py
### produces data into resultsdir

resultsdir = '/gpfs/data/greenocean/GCB/GCB2023/submissionData2023/'
...
# This is the top level directory where the runs are kept
...
runs = ["TOM12_TJ_GEA0", "TOM12_TJ_GEB0", "TOM12_TJ_GEC0", "TOM12_TJ_GED0"] 
    # Simulation ids (named after the year submission year, not the final year of simulation)
sim = ["A","B","C","D"]
...
forcedir = "/gpfs/data/greenocean/software/products/ERA5_v202303_TJ/" #change to ERA forcing
...
print("To run this paths and run ids needs to be specified in the code")

# origin = datetime.datetime(1980,1,1,0,0,0) # RECCAP
# time_origin_string = "1980-JAN-01 00:00:00" #RECCAP
# time_origin_units = "days since 1980-01-01 00:00:00"

# origin = datetime.datetime(1948,1,1,0,0,0) # BODC
# time_origin_string = "1948-JAN-01 00:00:00" # BODC
# time_origin_units = "days since 1948-01-01 00:00:00"
# ---------------------------------------

# The syntax of this is to add a number for which variable we want to produce
OutputNumber = int(sys.argv[1])
print('Output number is: ', OutputNumber)
...
if OutputNumber == -1:
    ...
    for r in range(0,len(runs)):
        ...
        outputFile = f'{resultsdir}integrated_timelines_PlankTOM_{sim[r]}_1_gr_{yrFrom}-{yrTo}_v{timestamp}.nc'
...
# if OutputNumber == 5: var.append( {'name':'epc1000',       'source':['POC'],                 'file':'ptrc',  'units': 'mol/L',       'dims': 2    , 'depth': 21 }  ) 
...
if OutputNumber == 8: var.append( {'name':'tos',          'source':['tos'],            'file':'grid',  'units': 'degC',       'dims': 2   , 'factor':1}  )
if OutputNumber == 9: var.append( {'name':'sos',          'source':['sos'],            'file':'grid',  'units': '1e-3',       'dims': 2   , 'factor':1}  )
...
if OutputNumber == 20: var.append( {'name':'mld',          'source':['mldr10_1'],            'file':'grid', 'units': 'm',       'dims': 2   , 'factor':1}  )
...
if OutputNumber == 24: var.append( {'name':'intd24ic',          'source':['DIC'],                 'file':'ptrc',  'units': 'mol/m2',       'dims': 2,  'depth': 'all'   , 'factor':1000}  )
...
if OutputNumber == 28: var.append( {'name':'thetao',       'source':['votemper'],            'file':'grid',  'units': 'degC',       'dims': 3   , 'factor':1}  )
if OutputNumber == 29: var.append( {'name':'so',           'source':['vosaline'],            'file':'grid',  'units': '1e-3',       'dims': 3   , 'factor':1}  )
...
if OutputNumber > -1:
    ...
    for v in var: 
        ...
        for r in range(0,len(runs)):
            ...
            outputFile = f'{resultsdir}{parm}_PlankTOM_{sim[r]}_1_gr_{yrFrom}-{yrTo}_v{timestamp}.nc'
