...
!         IF( .NOT.ln_traqsr )   RETURN
      ...
      !                                        ************************************
        ...
        parlux = (1.-rn_abs)/2.
        DO jj = 1, jpj !2, jpjm1
          DO ji = 1, jpi !fs_2, fs_jpim1   ! vector opt.
        ...
        DO jk = 1, jpkm1
            DO jj = 2, nlcj !jpjm1
               DO ji = 2, nlci !fs_2, fs_jpim1   ! vector opt.
               ekr = rn_ekwred
               ekg = rn_ekwgrn
...
! values from Manizza et al. 2005
               eliblu(ji,jj,jk+1) = eliblu(ji,jj,jk)*exp(-ekg*fse3t(ji,jj,jk)/2.)
               elired(ji,jj,jk+1) = elired(ji,jj,jk)*exp(-ekr*fse3t(ji,jj,jk)/2.)
               etot(ji,jj,jk) = eliblu(ji,jj,jk+1)+elired(ji,jj,jk+1)
               eliblu(ji,jj,jk+1) = eliblu(ji,jj,jk)*exp(-ekg*fse3t(ji,jj,jk))
               elired(ji,jj,jk+1) = elired(ji,jj,jk)*exp(-ekr*fse3t(ji,jj,jk))
            END DO
          END DO
        END DO
!      IF (lwp) WRITE(numout,*) 'traqsr ',qsr(nlci,nlcj),etot(nlci,nlcj,1),eliblu(nlci,nlcj,1),parlux,nlci,nlcj,jpi,jpj
      IF( ln_qsr_sms ) THEN
        DO jk = 1, jpkm1
            DO jj = 2, nlcj !jpjm1
               DO ji = 2, nlci !fs_2, fs_jpim1   ! vector opt.
               eliird= (qsr(ji,jj)*rn_abs)*exp(-fsdepw(1,1,jk)/rn_si0)
               etot3(ji,jj,jk+1) = eliird+eliblu(ji,jj,jk+1)+elired(ji,jj,jk+1)
               zta = r1_rau0_rcp/fse3t(ji,jj,jk)*(etot3(ji,jj,jk)*tmask(ji,jj,jk) &
