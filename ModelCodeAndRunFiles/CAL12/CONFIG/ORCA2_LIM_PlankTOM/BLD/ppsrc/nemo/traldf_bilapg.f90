...
!   'traldf_c2d' :                           eiv: 2D coefficient
   !!----------------------------------------------------------------------
   !! NEMO/OPA 3.3 , NEMO Consortium (2010)
   !! $Id: traldf_bilapg.F90 5147 2015-03-13 10:01:32Z cetlod $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
   ...
   SUBROUTINE ldfght ( kt, cdtype, pt, plt, kjpt, kaht )
      ...
      DO jn = 1, kjpt
         ...
         DO jk = 1, jpkm1                ! First step !   ! Horizontal slab
            ...
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
                  zabe1 = re2u_e1u(ji,jj) * e3u_0(ji,jj,jk)
                  zabe2 = re1v_e2v(ji,jj) * e3v_0(ji,jj,jk)
         ...
         DO jj = 2, jpjm1              !  Second step !   ! Horizontal slab
            ...
            DO jk = 2, jpkm1
               DO ji = 2, jpim1
                  zcof0 = e12t(ji,jj) / e3w_0(ji,jj,jk)   &
            ...
            IF( kaht == 1 ) THEN
               ...
               DO jk = 1, jpkm1
                  DO ji = 2, jpim1
                     ...
                     ! vertical divergence
                     ztav = rldf * ahtt(ji,jj) * ( zftw(ji,jk) - zftw(ji,jk+1) )
