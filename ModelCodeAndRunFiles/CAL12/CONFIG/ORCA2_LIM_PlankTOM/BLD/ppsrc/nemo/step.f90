   ...
   SUBROUTINE stp( kstp )
      ...
      IF( lk_dynspg_ts ) THEN 
          ...
          ! is necessary to compute momentum advection for the rhs of barotropic loop:
            IF(ln_sto_eos ) CALL sto_pts( tsn )                             ! Random T/S fluctuations
                            CALL eos    ( tsn, rhd, rhop, gdept_0(:,:,:) ) ! now in situ density for hpg computation
            IF( ln_zps .AND. .NOT. ln_isfcav)                               &
      ...
      IF( ln_traqsr .OR. lk_planktom ) CALL tra_qsr( kstp )       ! penetrative solar radiation qsr
      ...
      IF( ln_dynhpg_imp  ) THEN                             ! semi-implicit hpg (time stepping then eos)
         IF( ln_zdfnpc   )   CALL tra_npc( kstp )                ! update after fields by non-penetrative convection
            ...
            IF( ln_sto_eos ) CALL sto_pts( tsn )                 ! Random T/S fluctuations
                             CALL eos    ( tsa, rhd, rhop, gdept_0(:,:,:) )  ! Time-filtered in situ density for hpg computation
            IF( ln_zps .AND. .NOT. ln_isfcav)                                &
