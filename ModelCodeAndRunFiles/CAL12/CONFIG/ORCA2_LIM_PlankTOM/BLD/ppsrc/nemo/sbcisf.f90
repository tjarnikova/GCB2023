...
!     have to be replaced in the code by xxx_n
   ...
   !! $Id: sbcisf.F90 5540 2015-07-02 15:11:23Z jchanut $
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_isf in configuration namelist', lwp )
         ...
         DO jj = 1,jpj
            DO ji = 1,jpi
               ...
               rhisf_tbl(ji,jj) = MAX(rhisf_tbl_0(ji,jj), e3t_0(ji,jj,ikt))

               ! determine the deepest level influenced by the boundary layer
               ! test on tmask useless ?????
               DO jk = ikt, mbkt(ji,jj)
                  IF ( (SUM(e3t_0(ji,jj,ikt:jk-1)) .LT. rhisf_tbl(ji,jj)) .AND. (tmask(ji,jj,jk) == 1) ) ikb = jk
               END DO
               rhisf_tbl(ji,jj) = MIN(rhisf_tbl(ji,jj), SUM(e3t_0(ji,jj,ikt:ikb)))  ! limit the tbl to water thickness.
               misfkb(ji,jj) = ikb                                                  ! last wet level of the tbl
               r1_hisf_tbl(ji,jj) = 1._wp / rhisf_tbl(ji,jj)

               zhk           = SUM( e3t_0(ji, jj, ikt:ikb - 1)) * r1_hisf_tbl(ji,jj)  ! proportion of tbl cover by cell from ikt to ikb - 1
               ralpha(ji,jj) = rhisf_tbl(ji,jj) * (1._wp - zhk ) / e3t_0(ji,jj,ikb)  ! proportion of bottom cell influenced by boundary layer
            END DO
         END DO
   ...
   !!----------------------------------------------------------------------
    ...
    ! 3. -----------the average temperature between 200m and 600m ---------------------
             ...
             ! Calculate freezing temperature
                zpress = grav*rau0*gdept_0(ji,jj,ik)*1.e-04 
                CALL eos_fzp(tsb(ji,jj,ik,jp_sal), zt_frz, zpress) 
                zt_sum = zt_sum + (tsn(ji,jj,ik,jp_tem)-zt_frz) * e3t_0(ji,jj,ik) * tmask(ji,jj,ik)  ! sum temp
             ENDDO
...
!CDIR COLLAPSE
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            zpress(ji,jj)=grav*rau0*gdepw_0(ji,jj,mikt(ji,jj))*1.e-04
            !
         END DO
      END DO
...
! If non conservative we have zcfac=0.0 so what follows is then zfwflx*sss_m/zsfrz
   ...
   !      zvt = 0.5 * ( vtbl(ji  ,jj-1) + vtbl(ji,jj) )
      ...
      !! compute Monin Obukov Length
               ...
               zhmax = gdept_0(ji,jj,mbkt(ji,jj)) - gdepw_0(ji,jj,mikt(ji,jj)) -0.001
      ...
      DO jj = 2,jpj
         DO ji = 2,jpi
            IF (ssmask(ji,jj) == 1) THEN
               ...
               zhk = SUM( e3t_0(ji, jj, ikt:ikb - 1)) * r1_hisf_tbl(ji,jj)
               IF (cptin == 'T') &
