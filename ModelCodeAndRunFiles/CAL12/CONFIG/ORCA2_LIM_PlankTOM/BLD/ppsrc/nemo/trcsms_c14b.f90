...
MODULE trcsms_c14b
   ...
   INTEGER , PUBLIC, PARAMETER ::   jpmaxrec  = 400
   INTEGER , PUBLIC, PARAMETER ::   jpmaxrec2 = 4020!2 * jpmaxrec  ! 
...
!           &     + spco2(iyear_end2) * FLOAT( in2 ) ) / 6.
     ...
     &        * tmask(ji,jj,1) * ( 1. - fr_i(ji,jj) ) / max(trn(ji,jj,1,jpdic)*1e3,rtrn)
            ...
            tra(ji,jj,1,jpc14) = tra(ji,jj,1,jpc14) + qtr_c14(ji,jj) / e3t_0(ji,jj,1) 
     ...
     &        * tmask(ji,jj,1) * ( 1. - fr_i(ji,jj) ) / max(trn(ji,jj,1,jpdic)*1e3,rtrn)
            ! Add the surface flux to the trend
            tra(ji,jj,1,jpb14) = tra(ji,jj,1,jpb14) + qtr_b14(ji,jj) / e3t_0(ji,jj,1)
            qtr_d14(ji,jj) = -zpv * zsol * zpco2at                       &
     &                       * ( trb(ji,jj,1,jpd14) - zatmbb14(ji,jj) )  &
     &        * tmask(ji,jj,1) * ( 1. - fr_i(ji,jj) ) / max(trn(ji,jj,1,jpdic)*1e3,rtrn)
            ! Add the surface flux to the trend
            tra(ji,jj,1,jpd14) = tra(ji,jj,1,jpd14) + qtr_d14(ji,jj) / e3t_0(ji,jj,1)
            !
         END DO
      END DO
