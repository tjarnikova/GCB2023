...
CONTAINS
      ...
      CASE( jpdyn_zdf )                                                      ! Vertical Diffusion 
         ...
         DO jj = 2, jpjm1                                                             ! wind stress trends
            DO ji = 2, jpim1   ! vector opt.
               ztswu(ji,jj) = 0.5 * ( utau_b(ji,jj) + utau(ji,jj) ) / ( e3u_0(ji,jj,1) * rau0 )
               ztswv(ji,jj) = 0.5 * ( vtau_b(ji,jj) + vtau(ji,jj) ) / ( e3v_0(ji,jj,1) * rau0 )
            END DO
         END DO
         !
   ...
   SUBROUTINE trd_vor_iom( kt )
      ...
      DO jk = 1, jpk - 1
         zun(:,:) = zun(:,:) + e1u(:,:) * un(:,:,jk) * e3u_0(:,:,jk)
         zvn(:,:) = zvn(:,:) + e2v(:,:) * vn(:,:,jk) * e3v_0(:,:,jk)
      END DO
