...
!     have to be replaced in the code by xxx_n
   ...
   !! $Id: zdftke.F90 5407 2015-06-11 19:13:22Z smasson $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
...
!!bfr   - end commented area
      ...
      IF( ln_lc ) THEN      !  Langmuir circulation source term added to tke       (Axell JGR 2002)
         ...
         zpelc(:,:,1) =  MAX( rn2b(:,:,1), 0._wp ) * gdepw_0(:,:,1) * e3w_0(:,:,1)
         DO jk = 2, jpk
            zpelc(:,:,jk)  = zpelc(:,:,jk-1) + MAX( rn2b(:,:,jk), 0._wp ) * gdepw_0(:,:,jk) * e3w_0(:,:,jk)
         END DO
...
!CDIR NOVERRCHK
               DO ji = 2, jpim1   ! vector opt.
                  ...
                  zind = 0.5 - SIGN( 0.5, gdepw_0(ji,jj,jk) - zhlc(ji,jj) )
                  zwlc = zind * rn_lc * zus * SIN( rpi * gdepw_0(ji,jj,jk) / zhlc(ji,jj) )
...
!CDIR NOVERRCHK
         ...
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  ...
                  zmxlm(ji,jj,jk) = zemxl * wmask(ji,jj,jk) + MIN(zmxlm(ji,jj,jk),e3w_0(ji,jj,jk)) * (1 - wmask(ji,jj,jk))
                  zmxld(ji,jj,jk) = zemxl * wmask(ji,jj,jk) + MIN(zmxlm(ji,jj,jk),e3w_0(ji,jj,jk)) * (1 - wmask(ji,jj,jk))
               END DO
            END DO
         END DO
         !
      ...
      CASE ( 2 )           ! |dk[xml]| bounded by e3t :
         DO jk = 2, jpkm1         ! from the surface to the bottom :
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zmxlm(ji,jj,jk) = MIN( zmxlm(ji,jj,jk-1) + e3t_0(ji,jj,jk-1), zmxlm(ji,jj,jk) )
               END DO
            END DO
         END DO
      ...
      CASE ( 3 )           ! lup and ldown, |dk[xml]| bounded by e3t :
         DO jk = 2, jpkm1         ! from the surface to the bottom : lup
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zmxld(ji,jj,jk) = MIN( zmxld(ji,jj,jk-1) + e3t_0(ji,jj,jk-1), zmxlm(ji,jj,jk) )
               END DO
            END DO
         END DO
...
!CDIR NOVERRCHK
      ...
      IF( nn_pdl == 1 ) THEN      !* Prandtl number case: update avt
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zcoef = avm(ji,jj,jk) * 2._wp * e3w_0(ji,jj,jk) * e3w_0(ji,jj,jk)
                  !                                          ! shear
