...
!     have to be replaced in the code by xxx_n
   ...
   !! $Id: limsbc_2.F90 5407 2015-06-11 19:13:22Z smasson $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
   ...
   SUBROUTINE lim_sbc_init_2
      ...
      IF( nn_ice_embd == 2 .AND.          &  ! full embedment (case 2) & no restart : 
         ...
         do jk = 1,jpkm1                     ! adjust initial vertical scale factors
          e3t_0(:,:,jk) = e3t_0(:,:,jk)*( 1._wp + sshn(:,:)*tmask(:,:,1)/(ht_0(:,:) + 1.0 - tmask(:,:,1)) )
          e3t_0(:,:,jk) = e3t_0(:,:,jk)*( 1._wp + sshb(:,:)*tmask(:,:,1)/(ht_0(:,:) + 1.0 - tmask(:,:,1)) )
         end do
         e3t_0(:,:,:) = e3t_0(:,:,:)
         ...
         DO jk = 2, jpk
            gdept_0(:,:,jk) = gdept_0(:,:,jk-1) + e3w_0(:,:,jk)
            gdepw_0(:,:,jk) = gdepw_0(:,:,jk-1) + e3t_0(:,:,jk-1)
            gdep3w_0(:,:,jk) = gdept_0(:,:,jk  ) - sshn   (:,:)
         END DO
      ENDIF
      !
