...
CONTAINS
   ...
   SUBROUTINE zdf_tmx( kt )
      ...
      DO jk = 2, jpkm1
         zkz(:,:) = zkz(:,:) + e3w_0(:,:,jk) * MAX( 0.e0, rn2(:,:,jk) ) * rau0 * zav_tide(:,:,jk) * wmask(:,:,jk)
      END DO
      ...
      IF( kt == nit000 ) THEN       !* check at first time-step: diagnose the energy consumed by zav_tide
         ...
         DO jk= 1, jpk
            DO jj= 1, jpj
               DO ji= 1, jpi
                  ztpc = ztpc + e3w_0(ji,jj,jk) * e1t(ji,jj) * e2t(ji,jj)   &
...
!CDIR NOVERRCHK
      ...
      zsum2(:,:) = 0.e0
      DO jk= 2, jpk
         zsum1(:,:) = zsum1(:,:) + zempba_3d_1(:,:,jk) * e3w_0(:,:,jk) * tmask(:,:,jk) * tmask(:,:,jk-1)
         zsum2(:,:) = zsum2(:,:) + zempba_3d_2(:,:,jk) * e3w_0(:,:,jk) * tmask(:,:,jk) * tmask(:,:,jk-1)               
      END DO
      DO jj = 1, jpj
         DO ji = 1, jpi
      ...
      DO jk= 1, jpk
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               zsum     (ji,jj)    = zsum(ji,jj) + ztpc * e3w_0(ji,jj,jk)
            END DO
         END DO
       END DO
       DO jj = 1, jpj
      ...
      zkz(:,:) = 0.e0               ! Associated potential energy consummed over the whole water column
      DO jk = 2, jpkm1
         zkz(:,:) = zkz(:,:) + e3w_0(:,:,jk) * MAX( 0.e0, rn2(:,:,jk) ) * rau0 * zavt_itf(:,:,jk) * tmask(:,:,jk) * tmask(:,:,jk-1)
      END DO
      ...
      IF( kt == nit000 ) THEN       ! diagnose the nergy consumed by zavt_itf
         ...
         DO jk= 1, jpk
            DO jj= 1, jpj
               DO ji= 1, jpi
                  ztpc = ztpc + e1t(ji,jj) * e2t(ji,jj) * e3w_0(ji,jj,jk) * MAX( 0.e0, rn2(ji,jj,jk) )   &
