...
CONTAINS
   ...
   SUBROUTINE istate_t_s
      ...
      !
      DO jk = 1, jpk
         tsn(:,:,jk,jp_tem) = (  ( ( 7.5 - 0. * ABS( gphit(:,:) )/30. ) * ( 1.-TANH((gdept_0(:,:,jk)-80.)/30.) )   &
            &                + 10. * ( 5000. - gdept_0(:,:,jk) ) /5000.)  ) * tmask(:,:,jk)
   ...
   SUBROUTINE istate_eel
      ...
      SELECT CASE ( jp_cfg ) 
         ...
         CASE ( 5 )                                     ! EEL R5 configuration
            ...
            !
            DO jk = 1, jpk
               tsn(:,:,jk,jp_tem) = ( zt2 + zt1 * exp( - gdept_0(:,:,jk) / 1000 ) ) * tmask(:,:,jk)
               tsb(:,:,jk,jp_tem) = tsn(:,:,jk,jp_tem)
            END DO
            !
   ...
   SUBROUTINE istate_gyre
      ...
      CASE ( 0 )                  ! analytical T/S profil deduced from LEVITUS
         ...
         DO jk = 1, jpk
            DO jj = 1, jpj
               DO ji = 1, jpi
                  tsn(ji,jj,jk,jp_tem) = (  16. - 12. * TANH( (gdept_0(ji,jj,jk) - 400) / 700 )         )   &
                       &           * (-TANH( (500-gdept_0(ji,jj,jk)) / 150 ) + 1) / 2               &
                       &       + (      15. * ( 1. - TANH( (gdept_0(ji,jj,jk)-50.) / 1500.) )       &
                       &                - 1.4 * TANH((gdept_0(ji,jj,jk)-100.) / 100.)               &    
                       &                + 7.  * (1500. - gdept_0(ji,jj,jk)) / 1500.             )   & 
                       &           * (-TANH( (gdept_0(ji,jj,jk) - 500) / 150) + 1) / 2
                  tsn(ji,jj,jk,jp_tem) = tsn(ji,jj,jk,jp_tem) * tmask(ji,jj,jk)
                  tsb(ji,jj,jk,jp_tem) = tsn(ji,jj,jk,jp_tem)

                  tsn(ji,jj,jk,jp_sal) =  (  36.25 - 1.13 * TANH( (gdept_0(ji,jj,jk) - 305) / 460 )  )  &
                     &              * (-TANH((500 - gdept_0(ji,jj,jk)) / 150) + 1) / 2          &
                     &          + (  35.55 + 1.25 * (5000. - gdept_0(ji,jj,jk)) / 5000.         &
                     &                - 1.62 * TANH( (gdept_0(ji,jj,jk) - 60.  ) / 650. )       &
                     &                + 0.2  * TANH( (gdept_0(ji,jj,jk) - 35.  ) / 100. )       &
                     &                + 0.2  * TANH( (gdept_0(ji,jj,jk) - 1000.) / 5000.)    )  &
                     &              * (-TANH((gdept_0(ji,jj,jk) - 500) / 150) + 1) / 2 
   ...
   SUBROUTINE istate_uvg
      ...
      zprn(:,:,1) = zalfg * e3w_0(:,:,1) * ( 1 + rhd(:,:,1) )       ! Surface value

      DO jk = 2, jpkm1                                              ! Vertical integration from the surface
         zprn(:,:,jk) = zprn(:,:,jk-1)   &
            &         + zalfg * e3w_0(:,:,jk) * ( 2. + rhd(:,:,jk) + rhd(:,:,jk-1) )
      END DO  
