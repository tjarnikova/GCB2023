...
!     have to be replaced in the code by xxx_n
   ...
   !!----------------------------------------------------------------------
   !! NEMO/OPA 3.7 , NEMO Consortium (2014)
   !! $Id: trasbc.F90 5431 2015-06-16 17:33:42Z smasson $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
...
!!gm      IF( .NOT.ln_traqsr )   qsr(:,:) = 0.e0   ! no solar radiation penetration
      IF( .NOT.ln_traqsr ) THEN     ! no solar radiation penetration
         IF (mod(kt,nn_fsbc) .EQ. 1) qns(:,:) = qns(:,:) + qsr(:,:)      ! total heat flux in qns
!ET190925         qns(:,:) = qns(:,:) + qsr(:,:)
!ET190925         qsr(:,:) = 0.e0                     ! qsr set to zero
!         IF (lwp) WRITE(numout,*) 'trasbc ',qns(nlci,nlcj),qsr(nlci,nlcj)
      ENDIF
...
!                  zpress = grav*rau0*gdept_0(ji,jj,jk)*1.e-04
               ...
               ! compute tfreez for the temperature correction (we add water at freezing temperature)
!               zpress = grav*rau0*gdept_0(ji,jj,ikb)*1.e-04
