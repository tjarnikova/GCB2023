...
CONTAINS
   ...
   SUBROUTINE dom_vvl_init
      ...
      CALL dom_vvl_interpol( e3t_0(:,:,:), e3u_0(:,:,:), 'U' )
      CALL dom_vvl_interpol( e3t_0(:,:,:), e3v_0(:,:,:), 'V' )
      CALL dom_vvl_interpol( e3u_0(:,:,:), e3f_0(:,:,:), 'F' )
      ! Vertical scale factor interpolations
      ! ------------------------------------
      CALL dom_vvl_interpol( e3t_0(:,:,:), e3w_0(:,:,:), 'W'  )
      CALL dom_vvl_interpol( e3u_0(:,:,:), e3uw_0(:,:,:), 'UW' )
      CALL dom_vvl_interpol( e3v_0(:,:,:), e3vw_0(:,:,:), 'VW' )
      CALL dom_vvl_interpol( e3t_0(:,:,:), e3w_0(:,:,:), 'W'  )
      CALL dom_vvl_interpol( e3u_0(:,:,:), e3uw_0(:,:,:), 'UW' )
      CALL dom_vvl_interpol( e3v_0(:,:,:), e3vw_0(:,:,:), 'VW' )
      ...
      gdept_0(:,:,1) = 0.5_wp * e3w_0(:,:,1)
      gdepw_0(:,:,1) = 0.0_wp
      gdep3w_0(:,:,1) = gdept_0(:,:,1) - sshn(:,:)
      gdept_0(:,:,1) = 0.5_wp * e3w_0(:,:,1)
      gdepw_0(:,:,1) = 0.0_wp
         ...
         DO jj = 1,jpj
            DO ji = 1,jpi
              !    zcoef = (tmask(ji,jj,jk) - wmask(ji,jj,jk))   ! 0 everywhere tmask = wmask, ie everywhere expect at jk = mikt
               ...
               gdept_0(ji,jj,jk) =      zcoef  * ( gdepw_0(ji,jj,jk  ) + 0.5 * e3w_0(ji,jj,jk))  &
                   &                + (1-zcoef) * ( gdept_0(ji,jj,jk-1) +       e3w_0(ji,jj,jk)) 
               gdep3w_0(ji,jj,jk) = gdept_0(ji,jj,jk) - sshn(ji,jj)
               gdepw_0(ji,jj,jk) = gdepw_0(ji,jj,jk-1) + e3t_0(ji,jj,jk-1)
               gdept_0(ji,jj,jk) =      zcoef  * ( gdepw_0(ji,jj,jk  ) + 0.5 * e3w_0(ji,jj,jk))  &
                   &                + (1-zcoef) * ( gdept_0(ji,jj,jk-1) +       e3w_0(ji,jj,jk)) 
            END DO
      ...
      END DO
      hur(:,:) = umask_i(:,:) / ( hu(:,:) + 1. - umask_i(:,:) )
      hvr(:,:) = vmask_i(:,:) / ( hv(:,:) + 1. - vmask_i(:,:) )
   ...
   SUBROUTINE dom_vvl_sf_nxt( kt, kcall ) 
      ...
      DO jk = 1, jpkm1
         ...
         e3t_0(:,:,jk) = e3t_0(:,:,jk) + e3t_0(:,:,jk) * z_scale(:,:) * tmask(:,:,jk)
      END DO
      ...
      IF( ln_vvl_ztilde .OR. ln_vvl_layer .AND. ll_do_bclinic ) THEN   ! z_tilde or layer coordinate !
            ...
            IF( kt .GT. nit000 ) THEN
               DO jk = 1, jpkm1
                  hdiv_lf(:,:,jk) = hdiv_lf(:,:,jk) - rdt * frq_rst_hdv(:,:)   &
                     &          * ( hdiv_lf(:,:,jk) - e3t_0(:,:,jk) * ( hdivn(:,:,jk) - zhdiv(:,:) ) )
               END DO
            ENDIF
         END IF
         ...
         IF( ln_vvl_ztilde ) THEN     ! z_tilde case
            DO jk = 1, jpkm1
               tilde_e3t_a(:,:,jk) = tilde_e3t_a(:,:,jk) - ( e3t_0(:,:,jk) * ( hdivn(:,:,jk) - zhdiv(:,:) ) - hdiv_lf(:,:,jk) )
            END DO
         ELSE                         ! layer case
            DO jk = 1, jpkm1
               tilde_e3t_a(:,:,jk) = tilde_e3t_a(:,:,jk) -   e3t_0(:,:,jk) * ( hdivn(:,:,jk) - zhdiv(:,:) ) * tmask(:,:,jk)
            END DO
         END IF
         ...
         DO jk = 1, jpkm1
            dtilde_e3t_a(:,:,jk) = dtilde_e3t_a(:,:,jk) + e3t_0(:,:,jk) * z_scale(:,:) * tmask(:,:,jk)
         END DO
      ...
      !                                           ! ---baroclinic part--------- !
         DO jk = 1, jpkm1
            e3t_0(:,:,jk) = e3t_0(:,:,jk) + dtilde_e3t_a(:,:,jk) * tmask(:,:,jk)
         END DO
      ENDIF
   ...
   SUBROUTINE dom_vvl_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN        ! Read/initialise 
         ...
         IF( ln_rstart ) THEN                   !* Read the restart file
            ...
            IF( MIN( id1, id2 ) > 0 ) THEN       ! all required arrays exist
               CALL iom_get( numror, jpdom_autoglo, 'fse3t_b', e3t_0(:,:,:) )
               CALL iom_get( numror, jpdom_autoglo, 'fse3t_n', e3t_0(:,:,:) )
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN   ! Create restart file
         ...
         !                                           ! all cases !
         !                                           ! --------- !
         CALL iom_rstput( kt, nitrst, numrow, 'fse3t_b', e3t_0(:,:,:) )
         CALL iom_rstput( kt, nitrst, numrow, 'fse3t_n', e3t_0(:,:,:) )
         !                                           ! ----------------------- !
