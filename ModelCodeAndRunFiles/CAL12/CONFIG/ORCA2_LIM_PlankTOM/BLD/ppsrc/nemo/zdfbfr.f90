...
!! JC: possible WAD implementation should modify line below if layers vanish
                  ztmp = tmask(ji,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * e3t_0(ji,jj,ikbt) / rn_bfrz0 ))**2._wp
            ...
            IF ( ln_isfcav ) THEN
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     ikbt = mikt(ji,jj)
! JC: possible WAD implementation should modify line below if layers vanish
                     ztmp = (1-tmask(ji,jj,1)) * ( vkarmn / LOG( 0.5_wp * e3t_0(ji,jj,ikbt) / rn_bfrz0 ))**2._wp
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambfr in configuration namelist', lwp )
         ...
         IF ( ln_loglayer.AND.(.NOT.lk_vvl) ) THEN ! set "log layer" bottom friction once for all
            ...
            IF ( ln_isfcav ) THEN
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     ikbt = mikt(ji,jj)
                     ztmp = tmask(ji,jj,ikbt) * ( vkarmn / LOG( 0.5_wp * e3t_0(ji,jj,ikbt) / rn_tfrz0 ))**2._wp
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
             ...
             ikbv = mbkv(ji,jj)
             zfru = 0.5 * e3u_0(ji,jj,ikbu) / rdt
             zfrv = 0.5 * e3v_0(ji,jj,ikbv) / rdt
