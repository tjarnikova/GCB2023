...
!      DO jn = 1, jptra
      ...
      END DO
      DO jn = jpdsi, jpgoc
        IF( iom_varid( numrtr, 'sed_'//TRIM(ctrcnm(jn)), ldstop = .FALSE. ) > 0 ) THEN
          CALL iom_get( numrtr, jpdom_autoglo, 'sed_'//TRIM(ctrcnm(jn)), trnsed(:,:,jn) )
        ELSE
          trnsed(:,:,jn)=0.
        ENDIF
      END DO
...
!      DO jn = 1, jptra
      ...
      END DO
      DO jn = jpdsi, jpgoc
        CALL iom_rstput( kt, nitrst, numrtw, 'sed_'//TRIM(ctrcnm(jn)), trnsed(:,:,jn) )
      END DO
      !
      IF( kt == nitrst ) THEN
